...
class SavedFilter: NSObject {
    ...
    @objc var readableShortCutKey: String {
        get {
            return ([(.control, "⌃"), (.option, "⌥"), (.shift, "⇧"), (.command, "⌘")] as [(NSEvent.ModifierFlags, String)])
                    .map {
                        shortcutKeyModifiers.contains($0.0) ? $0.1 : ""
                    }
                    .joined()
    ...
    init?(dict: Any) {
        guard let dict = dict as? [String: String],
              ...
              let shortcutKeyModifiers = dict["shortcutKeyModifiers"] else {
            return nil
        }
        self.name = name
        self.filterString = filterString
        self.shortcutKey = shortcutKey
        self.shortcutKeyModifiers = shortcutKeyModifiers.compactMap {
            ModifierMap[$0]
        }.reduce([]) {
            $0.union($1)
        }
    ...
    func toDict() -> [String: String] {
        return [
            ...
            "shortcutKeyModifiers": String(ModifierMap.enumerated().compactMap {
                shortcutKeyModifiers.contains($0.element.value) ? $0.element.key : nil
            })
        ]
    }
}
