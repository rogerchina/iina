...
class AutoFileMatcher {
    ...
    private func getAllMediaFiles() {
        ...
        guard let files = try? fm.contentsOfDirectory(at: currentFolder, includingPropertiesForKeys: nil, options: searchOptions) else {
            return
        }
        ...
        // natural sort
        filesGroupedByMediaType[.video]!.sort {
            $0.filename.localizedStandardCompare($1.filename) == .orderedAscending
        }
        filesGroupedByMediaType[.audio]!.sort {
            $0.filename.localizedStandardCompare($1.filename) == .orderedAscending
        }
    }
    ...
    private func getAllPossibleSubs() -> [FileInfo] {
        ...
        let userDefinedSearchPaths = rawUserDefinedSearchPaths.components(separatedBy: ":").filter {
            !$0.isEmpty
        }
        for path in userDefinedSearchPaths {
            ...
            if hasWildcard {
                ...
                if let contents = try? fm.contentsOfDirectory(at: pathURL, includingPropertiesForKeys: [.isDirectoryKey], options: [.skipsHiddenFiles, .skipsPackageDescendants]) {
                    subDirs.append(contentsOf: contents.filter {
                        $0.isExistingDirectory
                    })
                }
            } else {
        ...
        for subDir in subDirs {
            if let contents = try? fm.contentsOfDirectory(at: subDir, includingPropertiesForKeys: nil, options: searchOptions) {
                subtitles.append(contentsOf: contents.compactMap {
                    subExts.contains($0.pathExtension.lowercased()) ? FileInfo($0) : nil
                })
            }
        }
    ...
    private func matchSubs(withMatchedSeries matchedPrefixes: [String: String]) throws {
        ...
        guard subAutoLoadOption != .disabled else {
            return
        }
        ...
        for video in filesGroupedByMediaType[.video]! {
            ...
            if subAutoLoadOption.shouldLoadSubsMatchedByIINA() {
                ...
                if !video.prefix.isEmpty, let matchedSubPrefix = matchedPrefixes[video.prefix] {
                    ...
                    for sub in subtitles {
                        guard let vn = video.nameInSeries, let sn = sub.nameInSeries else {
                            continue
                        }
            ...
            if let priorString = Preference.string(for: .subAutoLoadPriorityString), !matchedSubs.isEmpty {
                ...
                try matchedSubs
                        .filter {
                            $0.priorityStringOccurances > minOccurances
                        }  // eliminate false positives in filenames
                        .compactMap {
                            player.info.matchedSubs[video.path]!.index(of: $0.url)
                        }  // get index
    ...
    private func forceMatchUnmatchedVideos() throws {
        let unmatchedSubs = subtitles.filter {
            !$0.isMatched
        }
        ...
        if unmatchedSubs.count > 0 && unmatchedVideos.count > 0 {
            ...
            for sub in unmatchedSubs {
                ...
                for video in unmatchedVideos {
                    ...
                    sub.dist[video] = dist
                    video.dist[sub] = dist
                    if dist < minDistToVideo {
                        minDistToVideo = dist
                    }
                }
                guard minDistToVideo != .max else {
                    continue
                }
                sub.minDist = filesGroupedByMediaType[.video]!.filter {
                    sub.dist[$0] == minDistToVideo
                }
            }
            ...
            for video in unmatchedVideos {
                ...
                guard minDistToSub != .max else {
                    continue
                }
                try checkTicket()
                unmatchedSubs
                        .filter {
                            video.dist[$0]! == minDistToSub && $0.minDist.contains(video)
                        }
                        .forEach {
                            player.info.matchedSubs[video.path, default: []].append($0.url)
                        }
            }
        }
    }
