...
class PlayerCore: NSObject {
    ...
    static private func findIdlePlayerCore() -> PlayerCore? {
        return playerCores.first {
            $0.info.isIdle && !$0.info.fileLoading
        }
    }
    ...
    func uninitVideo() {
        guard mainWindow.isWindowLoaded else {
            return
        }
    ...
    func addToPlaylist(paths: [String], at index: Int) {
        getPlaylist()
        guard index <= info.playlist.count && index >= 0 else {
            return
        }
    ...
    func setAudioEq(fromGains gains: [Double]) {
        ...
        let filters = freqList.enumerated().map { (index, freq) -> MPVFilter in
            let string = [Int](0..<channelCount).map {
                "c\($0) f=\(freq) w=\(freq / 1.224744871) g=\(gains[index])"
            }.joined(separator: "|")
    ...
    func removeAudioEqFilter() {
        info.audioEqFilters?.compactMap {
            $0
        }.forEach {
            _ = removeAudioFilter($0)
        }
    ...
    func addVideoFilter(_ filter: MPVFilter) -> Bool {
        ...
        // try apply filter
        var result = true
        mpv.command(.vf, args: ["add", filter.stringFormat], checkError: false) {
            result = $0 >= 0
        }
    ...
    func removeVideoFilter(_ filter: MPVFilter) -> Bool {
        ...
        if let label = filter.label {
            mpv.command(.vf, args: ["del", "@" + label], checkError: false) {
                result = $0 >= 0
            }
        } else {
    ...
    func fileStarted() {
        ...
        guard let path = mpv.getString(MPVProperty.path) else {
            return
        }
        ...
        backgroundQueue.async {
            ...
            if let matchedSubs = self.info.matchedSubs[path] {
                ...
                for sub in matchedSubs {
                    guard currentTicket == self.backgroundQueueTicket else {
                        return
                    }
                    self.loadExternalSubFile(sub)
                }
                // set sub to the first one
                guard currentTicket == self.backgroundQueueTicket, self.mpv.mpv != nil else {
                    return
                }
    ...
    func sendOSD(_ osd: OSDMessage, autoHide: Bool = true, accessoryView: NSView? = nil) {
        ...
        guard mainWindow.isWindowLoaded && Preference.bool(for: .enableOSD) else {
            return
        }
    ...
    func getTrackInfo() {
        ...
        for index in 0..<trackCount {
            ...
            guard let trackType = mpv.getString(MPVProperty.trackListNType(index)) else {
                continue
            }
    ...
    func reloadSavedIINAfilters() {
        ...
        for filter in videoFilters {
            guard let label = filter.label else {
                continue
            }
            switch label {
    ...
    func checkCurrentMediaIsAudio() -> CurrentMediaIsAudioStatus {
        guard !info.isNetworkResource else {
            return .notAudio
        }
        ...
        let allVideoTracksAreAlbumCover = !info.videoTracks.contains {
            !$0.isAlbumart
        }
...
extension PlayerCore: FFmpegControllerDelegate {
    ...
    func didUpdate(_ thumbnails: [FFThumbnail]?, forFile filename: String, withProgress progress: Int) {
        guard let currentFilePath = info.currentURL?.path, currentFilePath == filename else {
            return
        }
