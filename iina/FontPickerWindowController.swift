...
class FontPickerWindowController: NSWindowController, NSTableViewDelegate, NSTableViewDataSource, NSTextFieldDelegate, NSControlTextEditingDelegate {
    ...
    override func windowDidLoad() {
        ...
        fontNames = manager.availableFontFamilies
                ...
                .map {
                    FontInfo(name: $0, localizedName: manager.localizedName(forFamily: $0, face: nil))
                }
                .sorted {
    ...
    func tableViewSelectionDidChange(_ notification: Notification) {
        guard let activeTv = notification.object as? NSTableView else {
            return
        }
        if activeTv == familyTableView {
            guard familyTableView.selectedRow >= 0 else {
                return
            }
    ...
    func controlTextDidChange(_ obj: Notification) {
        ...
        if str.isEmpty {
            ...
            filteredFontNames = fontNames.filter {
                $0.localizedName.lowercased().contains(str.lowercased())
            }
    ...
    private func updatePreview() {
        guard chosenFontMembers != nil else {
            return
        }
