...
class ThumbnailCache {
    ...
    static func fileIsCached(forName name: String, forVideo videoPath: URL?) -> Bool {
        ...
        if self.fileExists(forName: name) {
            ...
            let cacheVersion: Int = file.readData(ofLength: sizeofUInt8).withUnsafeBytes {
                $0.pointee
            }
            ...
            return file.readData(ofLength: sizeofUInt64).withUnsafeBytes {
                $0.pointee
            } == fileSize &&
                    file.readData(ofLength: sizeofInt64).withUnsafeBytes {
                        $0.pointee
                    } == fileTimestamp
        }
    ...
    static func read(forName name: String) -> [FFThumbnail]? {
        ...
        while file.offsetInFile != eof {
            ...
            let blockLength: Int64 = lengthData.withUnsafeBytes {
                $0.pointee
            }
            // timestamp
            let timestampData = file.readData(ofLength: sizeofDouble)
            let timestamp: Double = timestampData.withUnsafeBytes {
                $0.pointee
            }
            // jpeg
