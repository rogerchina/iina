...
extension PlayerCore {
    ...
    func hasPlayableFiles(in paths: [String]) -> Bool {
        for path in paths {
            if path.isDirectoryAsPath {
                ...
                guard let dirEnumerator = FileManager.default.enumerator(atPath: path) else {
                    return false
                }
    ...
    func getPlayableFiles(in urls: [URL]) -> [URL] {
        ...
        for url in urls {
            ...
            if url.hasDirectoryPath {
                ...
                guard let dirEnumerator = FileManager.default.enumerator(atPath: url.path) else {
                    return []
                }
                while let fileName = dirEnumerator.nextObject() as? String {
                    guard !fileName.hasPrefix(".") else {
    ...
    func isBDFolder(_ url: URL) -> Bool {
        ...
        guard bdmvFolder.isExistingDirectory else {
            return false
        }
    ...
    func acceptFromPasteboard(_ sender: NSDraggingInfo, isPlaylist: Bool = false) -> NSDragOperation {
        ...
        if types.contains(.nsFilenames) {
            guard var paths = pb.propertyList(forType: .nsFilenames) as? [String] else {
                return []
            }
    ...
    func openFromPasteboard(_ sender: NSDraggingInfo) -> Bool {
        ...
        if types.contains(.nsFilenames) {
            ...
            // try open files
            guard let loadedFileCount = openURLs(urls) else {
                return true
            }
            ...
            guard let url = pb.propertyList(forType: .nsURL) as? [String] else {
                return false
            }
            openURLString(url[0])
            return true
