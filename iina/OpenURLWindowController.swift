...
class OpenURLWindowController: NSWindowController, NSTextFieldDelegate, NSControlTextEditingDelegate {
    ...
    private func getURL() -> (url: URL?, hasScheme: Bool) {
        guard !urlField.stringValue.isEmpty else {
            return (nil, false)
        }
        ...
        guard let nsurl = NSURL(string: urlValue)?.standardized, let urlComponents = NSURLComponents(url: nsurl, resolvingAgainstBaseURL: false) else {
            return (nil, false)
        }
        if !username.isEmpty {
