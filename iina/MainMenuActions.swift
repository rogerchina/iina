...
class MainMenuActionHandler: NSResponder {
    ...
    @objc func menuDeleteCurrentFile(_ sender: NSMenuItem) {
        guard let url = player.info.currentURL else {
            return
        }
        do {
...
extension MainMenuActionHandler {
    ...
    @objc func menuFindOnlineSub(_ sender: NSMenuItem) {
        ...
        guard let url = player.info.currentURL, !player.isSearchingOnlineSubtitle else {
            return
        }
    ...
    @objc func saveDownloadedSub(_ sender: NSMenuItem) {
        let selected = player.info.subTracks.filter {
            $0.id == player.info.sid
        }
        guard let currURL = player.info.currentURL else {
            return
        }
