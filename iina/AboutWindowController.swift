...
class AboutWindowController: NSWindowController {
    ...
    override func windowDidLoad() {
        ...
        let gradient = NSGradient(colors: colors.map {
            NSColor(white: 0.925, alpha: $0)
        }, atLocations: loc, colorSpace: .deviceGray)
...
extension AboutWindowController: NSCollectionViewDataSource {
    ...
    func collectionView(_ collectionView: NSCollectionView, itemForRepresentedObjectAt indexPath: IndexPath) -> NSCollectionViewItem {
        ...
        guard let contributor = contributors[at: indexPath.item] else {
            return item
        }
    ...
    private func loadContributors(from url: String) {
        Just.get(url) { response in
            ...
            guard self.contributors.count > prevCount else {
                return
            }
        ...
        for langCode in languages {
            ...
            for (index, translator) in translators.enumerated() {
                ...
                result.append([
                    ...
                    "translator": ["\(translator.name)", urlString, emailString].compactMap {
                        $0
                    }.joined(separator: " ")
                ])
            }
