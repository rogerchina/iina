...
class MenuController: NSObject, NSMenuDelegate {
    ...
    func bindMenuItems() {
        ...
        // -- rotation
        let rotationTitles = AppData.rotations.map {
            "\($0)\(Constants.String.degree)"
        }
        ...
        // encoding
        let encodingTitles = AppData.encodings.map {
            $0.title
        }
        let encodingObjects = AppData.encodings.map {
            $0.code
        }
    ...
    func updateSavedFiltersMenu(type: String) {
        ...
        for item in menu.items {
            if let string = (item.representedObject as? String) {
                item.state = filters.contains {
                    $0.stringFormat == string
                } ? .on : .off
            }
        }
    }
    ...
    func updateKeyEquivalentsFrom(_ keyBindings: [KeyMapping]) {
        ...
        settings.forEach { (menuItem, isIINACmd, actions, normalizeLastNum, numRange, l10nKey) in
            ...
            for kb in keyBindings {
                guard kb.isIINACommand == isIINACmd else {
                    continue
                }
