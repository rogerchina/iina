...
class KeyMapping: NSObject {
    ...
    var rawAction: String {
        set {
            if newValue.hasPrefix("@iina") {
                ...
                action = rawAction.components(separatedBy: .whitespaces).filter {
                    !$0.isEmpty
                }
                isIINACommand = true
            } else {
    ...
    init(key: String, rawAction: String, isIINACommand: Bool = false, comment: String? = nil) {
        ...
        normalizedKey = normalizedKey.components(separatedBy: "+")
                .sorted {
                    KeyMapping.modifierOrder[$0, default: 9] < KeyMapping.modifierOrder[$1, default: 9]
                }
                .joined(separator: "+")
        if keyIsPlus {
            normalizedKey += "+"
        ...
        self.action = rawAction.components(separatedBy: .whitespaces).filter {
            !$0.isEmpty
        }
