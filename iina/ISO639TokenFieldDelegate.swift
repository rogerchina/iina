...
class ISO639TokenFieldDelegate: NSObject, NSTokenFieldDelegate {
    ...
    func tokenField(_ tokenField: NSTokenField, completionsForSubstring substring: String, indexOfToken tokenIndex: Int, indexOfSelectedItem selectedIndex: UnsafeMutablePointer<Int>?) -> [Any]? {
        ...
        let matches = ISO639Helper.languages.filter { lang in
            return lang.name.reduce(false) {
                $1.lowercased().hasPrefix(lowSubString) || $0
            }
        }
        return matches.map {
            $0.description
...
@objc(ISO639Transformer) class ISO639Transformer: ValueTransformer {
    ...
    override func transformedValue(_ value: Any?) -> Any? {
        ...
        return str.components(separatedBy: ",").map {
            ISO639TokenFieldDelegate.Token($0)
        }
    }

    override func reverseTransformedValue(_ value: Any?) -> Any? {
        guard let arr = value as? NSArray else {
            return ""
        }
        return arr.map {
            ($0 as! ISO639TokenFieldDelegate.Token).name
        }.joined(separator: ",")
    }
