...
class PreferenceWindowController: NSWindowController {
    ...
    class Trie {
        ...
        init(tab: String, section: String, label: String?) {
            s = [tab, section, label].compactMap {
                $0
    ...
    override func windowDidLoad() {
        ...
        let labelDict = [String: [String: [String]]](uniqueKeysWithValues: [
            ...
            (NSLocalizedString("preference.\($0[0])", comment: ""), self.getLabelDict(inNibNamed: $0[1]))
        })
    ...
    @IBAction func searchFieldAction(_ sender: Any) {
        ...
        if searchString.hasPrefix(lastString) {
            tries.filter {
                ...
                $0.search(String(searchString.dropFirst(lastString.count)))
            }
        } else {
            tries.forEach {
                $0.reset();
        ...
        currentCompletionResults = tries.filter {
            $0.active
        }.map {
            $0.returnValue
        }
    ...
    private func loadTab(at index: Int, thenFindLabelTitled title: String? = nil) {
        ...
        guard let vc = viewControllers[at: index] else {
            return
        }
...
extension PreferenceWindowController: NSTableViewDelegate, NSTableViewDataSource {
    ...
    func tableView(_ tableView: NSTableView, objectValueFor tableColumn: NSTableColumn?, row: Int) -> Any? {
        if tableView == self.tableView {
            ...
            guard let result = currentCompletionResults[at: row] else {
                return nil
            }
...
class PrefSearchResultMaskView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        guard let maskRect = maskRect else {
            return
        }
