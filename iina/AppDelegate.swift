...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationShouldTerminateAfterLastWindowClosed(_ sender: NSApplication) -> Bool {
        guard PlayerCore.active.mainWindow.isWindowLoaded || PlayerCore.active.initialWindow.isWindowLoaded else {
            return false
        }
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        guard !PlayerCore.active.mainWindow.isWindowHidden else {
    ...
    func handleOpenFile() {
        ...
        // open pending files
        let urls = pendingFilesForOpenFile.map {
    ...
    private func parsePendingURL(_ url: String) {
        ...
        if host == "open" || host == "weblink" {
            // open a file or link
            guard let queries = parsed.queryItems else {
                return
            }
            let queryDict = [String: String](uniqueKeysWithValues: queries.map {
                ($0.name, $0.value ?? "")
            })
            ...
            for query in queries {
                if query.name.hasPrefix("mpv_") {
                    ...
                    guard let mpvOptionValue = query.value else {
                        continue
                    }
    ...
    private func registerUserDefaultValues() {
        UserDefaults.standard.register(defaults: [String: Any](uniqueKeysWithValues: Preference.defaultPreference.map {
            ($0.0.rawValue, $0.1)
        }))
    }
