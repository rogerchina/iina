...
guard let lProjURLs = try? fm
        ...
        .filter {
    $0.hasDirectoryPath && $0.lastPathComponent.hasSuffix(".lproj") && $0.lastPathComponent != "Base.lproj"
...
let stringFileURLs = lProjURLs.map {
...
if CommandLine.arguments.count > 1 && CommandLine.arguments[1] == "--fix" {
    ...
    guard missingKeys.isEmpty else {
        error(missingKeys.map {
            "Missing key \"\($0.0)\" in \($0.1.path)."
        }.joined(separator: "\n"))
    }
}
