...
func makeSure(allKeysExistInFile file: String, basedOn base: BaseLang) {
    ...
    guard let baseDic = baseLang.directory.file(fullname).stringsContent else {
        return
    }
    ...
    for lang in testLanguages {
        ...
        guard var langDic = lang.directory.file(fullname).stringsContent else {
            stat[lang]! += 1;
            return
        }
        ...
        for (key, baseValue) in baseDic {
            ...
            if ignorePlaceHolderTitle && ignoredStrings.contains(baseValue) {
                continue
            }
        ...
        for (key, langValue) in langDic {
            guard checkRedundantKey else {
                continue
            }
            if ignorePlaceHolderTitle && (langValue == "Label" || langValue == "Text Cell") {
                continue
            }
...
let fileList = rawFileList.filter {
    !$0.hasPrefix(".")
}
