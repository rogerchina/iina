...
class MPVController: NSObject {
    ...
    func mpvInit() {
        ...
        var spdif: [String] = []
        if Preference.bool(for: PK.spdifAC3) {
            spdif.append("ac3")
        }
        if Preference.bool(for: PK.spdifDTS) {
            spdif.append("dts")
        }
        if Preference.bool(for: PK.spdifDTSHD) {
            spdif.append("dts-hd")
        }
    ...
    func mpvUninitRendering() {
        guard let mpvRenderContext = mpvRenderContext else {
            return
        }
    ...
    func getFilters(_ name: String) -> [MPVFilter] {
        ...
        guard let filters = (try? MPVNode.parse(node)!) as? [[String: Any?]] else {
            return result
        }
    ...
    private func handleEvent(_ event: UnsafePointer<mpv_event>!) {
        ...
        case MPV_EVENT_START_FILE:
            ...
            guard getString(MPVProperty.path) != nil else {
                break
            }
            player.fileStarted()
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        guard !(change?[NSKeyValueChangeKey.oldKey] is NSNull) else {
            return
        }
        ...
        guard let infos = optionObservers[keyPath] else {
            return
        }
