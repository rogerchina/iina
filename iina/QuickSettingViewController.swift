...
class QuickSettingViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate, SidebarViewController {
    ...
    override func viewDidLoad() {
        ...
        observe(.iinaTracklistChanged) { _ in
            self.withAllTableViews { view, _ in
                view.reloadData()
            }
        }
        observe(.iinaVIDChanged) { _ in
            self.videoTableView.reloadData()
        }
        observe(.iinaAIDChanged) { _ in
    ...
    private func updateSubTabControl() {
        if let currSub = player.info.currentTrack(.sub) {
            ...
            [subTextColorWell, subTextSizePopUp, subTextBgColorWell, subTextBorderColorWell, subTextBorderWidthPopUp, subTextFontBtn].forEach {
                $0.isEnabled = enableTextSettings
            }
        }
    ...
    @IBAction func tabBtnAction(_ sender: NSButton) {
        ...
        [videoTabBtn, audioTabBtn, subTabBtn].forEach {
            Utility.setBoldTitle(for: $0, $0 == sender)
        }
    ...
    func showSubChooseMenu(forView view: NSView, showLoadedSubs: Bool = false) {
        ...
        let addMenuItem = { (sub: FileInfo) -> Void in
            let isActive = !showLoadedSubs && activeSubs.contains {
                $0.externalFilename == sub.path
            }
