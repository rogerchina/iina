...
class MainWindowController: NSWindowController, NSWindowDelegate {
    ...
    enum FullScreenState: Equatable {
        ...
        var priorWindowedFrame: NSRect? {
            ...
            set {
                guard let newRect = newValue else {
                    return
                }
                switch self {
        ...
        mutating func startAnimatingToWindow() {
            guard case .fullscreen(let legacy, let priorWindowedFrame) = self else {
                return
            }
    ...
    private func setupOSCToolbarButtons(_ buttons: [Preference.ToolBarButton]) {
        ...
        fragToolbarView.views.forEach {
            fragToolbarView.removeView($0)
        }
    ...
    private func setupOnScreenController(withPosition newPosition: Preference.OSCPosition) {
        ...
        guard !oscIsInitialized || oscPosition != newPosition else {
            return
        }
        ...
        if let cb = currentControlBar {
            ...
            fadeableViews = fadeableViews.filter {
                $0 != cb
            }
        }

        // reset
        ([controlBarFloating, controlBarBottom, oscTopMainView] as [NSView]).forEach {
            $0.isHidden = true
        }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        guard !controlBarFloating.isDragging else {
            return
        }
        ...
        if isDragging {
            ...
            if !isMouseEvent(event, inAnyOf: [sideBarView, subPopoverView]) && sideBarStatus != .hidden {
                ...
                if event.clickCount == 1 {
                    ...
                    guard !isMouseEvent(event, inAnyOf: [sideBarView, currentControlBar, titleBarView, subPopoverView]) else {
                        return
                    }
                    // single click
                    ...
                    guard !isMouseEvent(event, inAnyOf: [sideBarView, currentControlBar, subPopoverView]) else {
                        return
                    }
                    // double click
                    guard doubleClickAction != .none else {
                        return
                    }
    ...
    @objc private func performMouseActionLater(_ timer: Timer) {
        guard let action = timer.userInfo as? Preference.MouseClickAction else {
            return
        }
    ...
    override func scrollWheel(with event: NSEvent) {
        ...
        guard !isMouseEvent(event, inAnyOf: [sideBarView, titleBarView, subPopoverView]) else {
            return
        }
        if isMouseEvent(event, inAnyOf: [currentControlBar]) && !isMouseEvent(event, inAnyOf: [fragVolumeView, fragSliderView]) {
            return
        }
    ...
    @objc func handleMagnifyGesture(recognizer: NSMagnificationGestureRecognizer) {
        guard pinchAction != .none else {
            return
        }
        guard !isInInteractiveMode, let window = window, let screenFrame = NSScreen.main?.visibleFrame else {
            return
        }
    ...
    func windowDidOpen() {
        ...
        guard let w = self.window, let cv = w.contentView else {
            return
        }
    ...
    func displayOSD(_ message: OSDMessage, autoHide: Bool = true, accessoryView: NSView? = nil) {
        guard player.displayOSD && !isShowingPersistentOSD else {
            return
        }
        ...
        [osdAccessoryText, osdAccessoryProgress].forEach {
            $0.isHidden = true
        }
    ...
    private func removeTitlebarViewFromFadeableViews() {
        if let index = (self.fadeableViews.index {
            $0 === titleBarView
        }) {
    ...
    private func setMaterial(_ theme: Preference.Theme?) {
        guard let window = window, let theme = theme else {
            return
        }
    ...
    func setWindowScale(_ scale: Double) {
        guard let window = window, fsState == .windowed else {
            return
        }
    ...
    private func blackOutOtherMonitors() {
        screens = NSScreen.screens.filter {
            $0 != window?.screen
        }
    ...
    func toggleWindowFullScreen() {
        ...
        switch fsState {
        case .windowed:
            guard !player.isInMiniPlayer else {
                return
            }
        ...
        if andProgressBar {
            ...
            if #available(macOS 10.12.2, *) {
                ...
                player.touchBarSupport.touchBarPosLabels.forEach {
                    $0.updateText(with: duration, given: pos)
                }
            }
        }
    }
    ...
    @objc func toolBarButtonAction(_ sender: NSButton) {
        guard let buttonType = Preference.ToolBarButton(rawValue: sender.tag) else {
            return
        }
        switch buttonType {
        case .fullScreen:
